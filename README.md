...
| 数据缓存 | 内存 + 磁盘 JSON（按交易日） | 内存 + `chrome.storage.local` | 磁盘 JSON（申万映射/交易日历 24h） |
| 交易日历 | ✅ akshare 交易日历（识别节假日） | ✅ 新浪交易日历（识别节假日） | ✅ 新浪日K线交易日历（识别节假日） |
...
**🤖 Skill 版界面**（筛选器内嵌页面：分组方式 / 色块大小 / Top-N / 包含排除筛选，全部在浏览器内实时切换，色块标注涨跌幅）

![Skill 版视图](docs/screenshots/skill.jpg)
...
| 行情数据源 | 腾讯行情 + 新浪指数 + akshare | 腾讯行情 + 三交易所官方接口 + 申万官网 + 新浪交易日历 | 腾讯行情 + 申万官网 + 新浪日K线 |
...
| **交易日历（节假日）** | ✅ akshare 交易日历 | ✅ 新浪交易日历（前端解码） | ✅ 新浪上证指数日K线（明文日期，免解码） |
| **内嵌筛选器** | ❌ 需刷新请求 | ❌ 需刷新请求 | ✅ 浏览器内实时切换 |
| 强制刷新限流 | ✅ 同 IP 30s/次 | —（本地使用无需） | —（本地脚本无需） |
| 多用户/公网共享 | ✅ 适合 | ⛔ 仅本机 | ⛔ 仅本机 |
| 部署成本 | 需 Python 环境 + 可选 Nginx | 零部署，加载即用 | 需 Python 环境（标准库） |

> 三版均能**识别法定节假日**：网页版借助 akshare 交易日历；插件版纯前端直连新浪同源交易日历接口（`klc_td_sh.txt`），用内置解码器还原出当年完整交易日并缓存到 `chrome.storage.local`；Skill 版直连新浪上证指数日K线接口，用真实交易日日期构建日历（明文 JSON，免解码），磁盘缓存 24h。任一日历接口拉取失败，都会自动退回「仅排除周末」逻辑，不影响数据正确性。
...
![Skill 版效果图](docs/screenshots/skill.jpg)

> 全市场 5551 只按**申万一级行业**分组、色块大小 = 总市值、颜色 = 涨跌幅（红涨绿跌）、色块上直接标注涨跌幅数值；页面顶部筛选器（分组方式 / 色块大小 / Top-N / 包含与排除）全部在浏览器内实时生效，支持全屏与导出 PNG。
...
│   ├── sector_map.json             #   申万映射缓存（运行时自动生成）
│   └── trade_calendar.json         #   交易日历缓存（运行时自动生成）
├── assets/
...
- **交易日历（识别节假日）**：直连新浪上证指数日K线接口构建交易日集合（明文日期免解码），正确识别周末与法定节假日，磁盘缓存 24h，失败自动退回「仅排除周末」。
...
- **依赖最小**：脚本版为降低依赖，代码列表用交易所板块代码段近似兜底（实测覆盖约 5500 只真实股票）；交易日历用新浪上证指数日K线（明文日期，免解码），接口失败时同样退回「仅排除周末」。
...
- 三版均识别法定节假日：网页版用 akshare 日历，插件版用新浪日历（前端解码），Skill 版用新浪上证指数日K线（明文日期）。任一获取失败都会自动退回「仅排除周末」逻辑。
